...
// NEW: Check if block is active (with 10-minute buffer before first trip)
function isBlockActive(blockId, scheduleData, currentScheduleSec, operatorId = '36') {
  ...
  // Block is active if current time is between (first trip start - 10 minutes) and last trip end
  const buffer = 600; // 10 minutes before first trip
...
function findCurrentOrRecentTripInBlock(blockId, scheduleData, currentScheduleSec, operatorId = '36') {
  ...
  // Check if we're before the first trip (with 10-minute buffer)
  const firstTrip = tripsInBlock[0];
...
function calculateVirtualBusPositionForBlock(blockId, scheduleData, currentTimeSec, operatorId = '36') {
  ...
  if (tripInfo.isDuringTrip) {
    ...
    // Get static stop times for the relevant trip
    const staticStopTimes = getStaticScheduleForTrip(tripInfo.tripId, scheduleData);
    if (!staticStopTimes || staticStopTimes.length === 0) {
      console.log(`[calculateVirtualBusPositionForBlock] No stop times for trip ${tripInfo.tripId} in block ${blockId}`);
      return null;
    }
    ...
    if (tripInfo.timeUntilTrip) {
      // Before first trip: position at FIRST stop of the first trip
      stop = staticStopTimes[0];
      progress = 0;
      status = 'BEFORE_FIRST_TRIP';
      timeInfo = `Starts in ${Math.round(tripInfo.timeUntilTrip / 60)} min`;
    } else {
      // Layover after a trip: position at LAST stop of the previous trip (which is start of next)
    ...
    if (!stopCoords) {
      console.log(`[calculateVirtualBusPositionForBlock] No coordinates for stop ${stop.stop_id} in block ${blockId}`);
      return null;
    }
